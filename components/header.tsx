...
const navigation = [
    { name: "Hem", href: "/" },
    { name: "Här börjar ni", href: "/sell-with-us" },
    { name: "Produkter", href: "/products" },
    { name: "Vinst", href: "/profit" },
...
export function Header() {
    ...
    return (
        <header className={`sticky top-0 z-50 w-full border-b transition-all duration-300  ${scrolled ? "border-border bg-background shadow-sm py-2" : "border-border/40 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60 py-4"}`}>
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex  items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center space-x-8">
                        <Link href="/dashboard" title="Översikt" className="cursor-pointer">
                        ...
                        {user ? (
                            <div className="relative avatar-dropdown-container">
                                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center focus:outline-none transition-all duration-200 hover:scale-105 cursor-pointer" aria-haspopup="true" aria-expanded={isDropdownOpen} title="Användarmeny">
                                    {user.profileImage ? (
                                ...
                                {isDropdownOpen && (
                                    <div className="absolute right-0 mt-3 w-56 rounded-xl shadow-xl bg-white border border-gray-100 focus:outline-none z-50 overflow-hidden divide-y divide-gray-100 animate-in fade-in slide-in-from-top-2 duration-150">
                                        <div className="px-4 py-3 bg-gray-50/50">
                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Inloggad som</p>
