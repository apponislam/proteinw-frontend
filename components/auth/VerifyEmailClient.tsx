...
const VerifyEmailClient = () => {
    ...
    useEffect(() => {
        if (!token || !email) {
            setStatus("error");
            setErrorMessage("Ogiltiga eller saknade parametrar i verifieringslänken.");
            return;
        }
        ...
        if (isLoading) {
            ...
            setSuccessMessage(resData?.message || "E-postadressen har verifierats.");
        } else if (isError) {
            setStatus("error");
            const errData = error as any;
            setErrorMessage(errData?.data?.message || "Misslyckades att verifiera e-postadressen. Länken kan ha gått ut eller vara ogiltig.");
        }
    }, [isLoading, isSuccess, isError, data, error, token, email]);

    const handleResend = async () => {
        if (!email) {
            toast.error("Ingen e-postadress hittades för att skicka om länk.");
            return;
        }
        try {
            ...
            toast.error(err?.data?.message || "Misslyckades att skicka verifieringslänk.");
        }
    };
                    ...
                    <div className="bg-white border-dashed rounded-lg p-4 sm:p-8 shadow-sm">
                        ...
                        {status === "loading" && (
                            <div className="text-center py-6">
                                ...
                                <h2 className="text-2xl text-gray-900 font-bold mb-2">Verifierar e-post...</h2>
                                <p className="text-gray-600 text-sm">Vänta medan vi verifierar din e-postlänk.</p>
                            </div>
                        )}
                        ...
                        {status === "success" && (
                            <div className="text-center">
                                ...
                                <h2 className="text-2xl text-gray-900 font-bold mb-4">E-postadressen verifierad!</h2>
                        ...
                        {status === "error" && (
                            <div className="text-center">
                                ...
                                <h2 className="text-2xl text-gray-900 font-bold mb-4">Verifieringen misslyckades</h2>
                                ...
                                <div className="space-y-3">
                                    {email && (
                                        ...
                                        >
                                            {isResending ? "Skickar igen..." : "Skicka om verifieringslänk"}
                                        </button>
                                    )}
