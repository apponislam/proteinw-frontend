...
import { useGetMyCampaignsQuery, TCampaign } from "@/redux/features/campaign/campaignApi";

interface CampaignListOrdersPageProps {
    onSelectCampaign?: (campaign: TCampaign | null) => void;
    selectedCampaignId?: string;
    placeholder?: string;
}

const CampaignListOrdersPage: React.FC<CampaignListOrdersPageProps> = ({
    onSelectCampaign,
    selectedCampaignId,
    placeholder = "Alla försäljningar",
}) => {
    ...
    const { data: response, isFetching, isLoading, isError } = useGetMyCampaignsQuery({
        page,
        limit: 6,
        status: "ACTIVE",
    });
    ...
    useEffect(() => {
        if (newCampaigns.length > 0) {
            if (page === 1) {
                ...
                if (newCampaigns.length === 1 && !hasNextPage) {
                    if (onSelectCampaign) {
    ...
    }, [newCampaigns, page, hasNextPage, onSelectCampaign]);
...
export default CampaignListOrdersPage;
