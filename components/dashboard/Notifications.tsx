...
import { useGetAllContactsQuery, useGetUnreadCountQuery, useMarkAsReadMutation, useMarkAllAsReadMutation, useDeleteContactMutation } from "@/redux/features/contact/contactApi";
...
const Notifications: React.FC<NotificationsProps> = ({ isOpen, onClose }) => {
    ...
    const { data: contactsData, isLoading, isFetching, isError } = useGetAllContactsQuery({ page: 1, limit }, { skip: !isOpen });
    ...
    return (
        <>
            ...
            <div className="fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-[0px_0px_20px_0px_rgba(0,0,0,0.12)] z-50 flex flex-col animate-in slide-in-from-right duration-200">
                ...
                <div ref={containerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-6 space-y-4">
                    {isLoading ? (
                        ...
                        <>
                            {notifications.map((notification) => (
                                <div
                                    ...
                                    className={`rounded-lg p-4 border transition-all relative group cursor-pointer ${!notification.isRead ? "bg-[#D97706]/5 border-[#D97706]/20 hover:bg-[#D97706]/10" : "bg-[#FAFAF9] border-[#F5F5F4] hover:bg-[#F5F5F4]"}`}
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <span className="font-bold text-[#1A1C1C]">{notification.name}</span>
                                                {!notification.isRead && <span className="w-2 h-2 bg-[#D97706] rounded-full animate-pulse" />}
                                            </div>
                                    ...
                                    <div className="space-y-1 text-xs border-t border-[#F5F5F4] pt-2 mt-2">
                                        ...
                                        {notification.createdAt && <div className="text-[10px] text-gray-400 pt-1">{new Date(notification.createdAt).toLocaleString()}</div>}
                                    </div>
                                </div>
                            ))}
                            {isFetching && (
