...
interface CampaignListSellerProps {
    onSelectCampaign?: (campaign: TCampaign) => void;
    selectedCampaignId?: string;
}

const CamPaignListSeller: React.FC<CampaignListSellerProps> = ({ onSelectCampaign, selectedCampaignId }) => {
                        ...
                        {accumulatedCampaigns.map((campaign) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {campaign.status && <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ml-2 shrink-0 ${campaign.status === "ACTIVE" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-700"}`}>{campaign.status === "ACTIVE" ? "AKTIV" : campaign.status}</span>}
                                </button>
                            );
                        })}
...
export default CamPaignListSeller;
