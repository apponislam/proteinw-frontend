...
const SellerCampaignOrdersList: React.FC<SellerCampaignOrdersListProps> = ({ campaignId }) => {
    ...
    const handleStatusChange = async (orderId: string, newStatus: TOrderStatus) => {
        try {
            ...
            toast.success("Orderstatus har uppdaterats!");
        } catch (err: any) {
    ...
    if (isLoading) {
        return (
            <div className="p-8 text-center">
                ...
                <p className="text-[#78716C] text-sm font-medium">Laddar beställningar...</p>
            </div>
        );
    }
        ...
        <div className="w-full">
            ...
            <div className="p-4 border-b border-[#E7E5E4] bg-[#FAF9F6] rounded-t-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-bold text-[#1A1C1C] uppercase tracking-wider truncate">Mina kampanjbeställningar ({pagination.total})</span>
                </div>
            ...
            {ordersList.length === 0 ? (
                <div className="p-8 text-center text-sm text-[#78716C]">Inga försäljningar hittades för denna kampanj.</div>
            ) : (
                <div className="overflow-x-auto w-full">
                    <table className="w-full text-left border-collapse min-w-150">
                        ...
                        <tbody className="divide-y divide-[#E7E5E4] text-sm text-[#1A1C1C]">
                            {ordersList.map((order, index) => {
                                ...
                                const sellerName = (order as any).memberId?.name || (order as any).member?.name || "Gäst / Direkt";
                                ...
                                return (
                                    <tr key={order._id || index} onClick={() => setSelectedOrder(order)} className="hover:bg-[#FCFBFA] transition-colors whitespace-nowrap cursor-pointer">
                                        ...
                                        <td className="py-3.5 px-4 text-center">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold capitalize ${getStatusColor(order.status)}`}>{statusDisplayMap[order.status] || order.status}</span>
                                        </td>
                    ...
                    {pagination.totalPages && pagination.totalPages > 1 ? (
                        <div className="p-4 border-t border-[#E7E5E4]">
                            <Pagination meta={pagination} onPageChange={setPage} itemName="BESTALLNINGAR" />
                        </div>
                    ) : null}
                </div>
            )}
                    ...
                    <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 relative shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div className="space-y-3 bg-[#FAFAF9] p-4 rounded-xl border border-[#E7E5E4]">
                                ...
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Telefon:</span> {activeSelectedOrder.customerPhone || "Ej tillgänglig"}
                                </p>
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Adress:</span> {(activeSelectedOrder as any).deliveryAddress?.streetAddress || activeSelectedOrder.address?.street}, {(activeSelectedOrder as any).deliveryAddress?.city || activeSelectedOrder.address?.city},{" "}
                            ...
                            <div className="space-y-3 bg-[#FAFAF9] p-4 rounded-xl border border-[#E7E5E4]">
                                ...
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Säljarens namn:</span> {(activeSelectedOrder as any).memberId?.name || (activeSelectedOrder as any).member?.name || "Gäst / Direkt"}
                                </p>
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Säljarens e-post:</span> {(activeSelectedOrder as any).memberId?.email || (activeSelectedOrder as any).member?.email || "Ej tillgänglig"}
                                </p>
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Gruppnamn:</span> {(activeSelectedOrder as any).groupId?.name || (activeSelectedOrder as any).group?.name || "Ej tillgänglig"}
                                </p>
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Kampanjnamn:</span> {(activeSelectedOrder as any).campaignId?.name || (activeSelectedOrder as any).campaign?.name || "Ej tillgänglig"}
                                </p>
                            </div>
                        </div>
                                    ...
                                    <div key={index} className="flex justify-between items-center text-xs bg-gray-50 p-3 rounded-xl border border-gray-100">
                                        <div>
                                            ...
                                            <p className="text-[11px] text-gray-500">
                                                Antal: {item.quantity} x {item.singlePrice || (item as any).price} SEK
                                            </p>
                                        </div>
