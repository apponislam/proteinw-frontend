    ...
    return (
        <div className="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-[0px_0px_14px_0px_rgba(0,0,0,0.08)]">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
                <div>
                    <h2 className="text-xl font-bold text-[#1A1C1C]">Alla beställningar</h2>
                </div>
            ...
            {isLoading ? (
                <div className="text-center py-12">
                    ...
                    <p className="text-[#78716C] text-sm">Laddar beställningar...</p>
                </div>
            ) : ordersList.length === 0 ? (
                <div className="text-center py-12 text-[#78716C]">Inga beställningar hittades som matchar dina filter.</div>
            ) : (
                <>
                    ...
                    <div className="block md:hidden space-y-3">
                        {ordersList.map((order, index) => {
                            ...
                            const groupName = (order.groupId as any)?.name || "Ej tillgänglig";
                            ...
                            return (
                                <div key={order._id || index} onClick={() => setSelectedOrder(order)} className="bg-[#FAFAF9] hover:bg-[#FFDEA8] p-3.5 sm:p-4 rounded-xl border border-[#E7E5E4] transition-colors cursor-pointer space-y-3 shadow-2xs">
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize shrink-0 ${getStatusColor(order.status)}`}>{statusDisplayMap[order.status] || order.status}</span>
                                    </div>
                                    ...
                                    <div className="pt-2 border-t border-[#E7E5E4]/60 flex items-center justify-between text-xs gap-2">
                                        <div className="min-w-0 flex-1">
                                            <span className="text-stone-500 font-semibold block text-[10px] truncate">GRUPP: {groupName}</span>
                        ...
                        <table className="w-full min-w-225 text-left text-sm">
                            <thead>
                                <tr className="bg-[#FAFAF9]">
                                    ...
                                    <th className="px-4 py-3 text-[#78716C] text-xs font-medium uppercase tracking-wider">SÄLJARE</th>
                                    ...
                                    <th className="px-4 py-3 text-[#78716C] text-xs font-medium uppercase tracking-wider">PRODUKT</th>
                                    ...
                                    return (
                                        <tr key={order._id || index} onClick={() => setSelectedOrder(order)} className="border-b border-[#F5F5F4] last:border-0 hover:bg-[#FFDEA8] transition-colors duration-200 cursor-pointer">
                                            ...
                                            <td className="px-4 py-3.5 whitespace-nowrap">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusColor(order.status)}`}>{statusDisplayMap[order.status] || order.status}</span>
                                            </td>
                    ...
                    <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-4 md:p-6 relative shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="space-y-3 bg-[#FAFAF9] p-4 rounded-xl border border-[#E7E5E4]">
                                ...
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Säljarens e-post:</span> {(activeSelectedOrder.memberId as any)?.email || "Ej tillgänglig"}
                                </p>
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Gruppnamn:</span> {(activeSelectedOrder.groupId as any)?.name || "Ej tillgänglig"}
                                </p>
                                <p className="text-xs text-[#1A1C1C]">
                                    <span className="font-semibold text-gray-600">Kampanjnamn:</span> {(activeSelectedOrder.campaignId as any)?.name || "Ej tillgänglig"}
                                </p>
                            </div>
                        </div>
                                ...
                                <div className="flex items-center gap-3 relative">
                                    ...
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            <span className="text-gray-800">{statusDisplayMap[activeSelectedOrder.status] || activeSelectedOrder.status}</span>
