...
export const SellerCampaignOrders = ({ memberId, campaignId }: { memberId: string; campaignId?: string }) => {
    ...
    if (orders.length === 0) {
        return <div className="text-center py-8 text-[#78716C] bg-[#FAFAF9] rounded-lg border border-dashed border-[#E7E5E4] text-sm">Inga beställningar hittades för denna säljare.</div>;
    }
    ...
    return (
        <div className="mt-4">
                ...
                {orders.map((order) => (
                    <div key={order._id} className="p-3.5 rounded-xl border border-[#E7E5E4] bg-[#FAFAF9] space-y-2.5">
                        <div className="flex items-start justify-between gap-2">
                            ...
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold shrink-0 ${order.status === "delivered" ? "bg-green-100 text-green-800" : order.status === "cancelled" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`}>
                        ...
                        <div className="flex items-center justify-between text-xs pt-2 border-t border-stone-200/60 font-medium text-[#1A1C1C]">
                            <div>
                                <span className="text-[#78716C] font-normal">Paket: </span>
                                <span className="font-bold">{order.totalPackage}</span>
                            </div>
                            <div>
                                <span className="text-[#78716C] font-normal">Totalpris: </span>
                ...
                <table className="w-full text-left text-sm">
                    <thead>
                        <tr className="bg-[#FAFAF9] border-b border-[#E7E5E4]">
                            <th className="px-4 py-3 text-[#78716C] font-semibold text-xs uppercase tracking-wider">Kund</th>
                            <th className="px-4 py-3 text-[#78716C] font-semibold text-xs uppercase tracking-wider">Paket</th>
                            <th className="px-4 py-3 text-[#78716C] font-semibold text-xs uppercase tracking-wider">Totalpris</th>
                    ...
                    <tbody>
                        {orders.map((order) => (
                            <tr key={order._id} className="border-b border-[#F5F5F4] last:border-0 hover:bg-[#FFDEA8] transition-colors duration-150">
                                ...
                                <td className="px-4 py-3">
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${order.status === "delivered" ? "bg-green-100 text-green-800" : order.status === "cancelled" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`}>
                                        {formatStatus(order.status)}
                                    </span>
                                </td>
                            </tr>
            ...
            <Pagination meta={meta} onPageChange={setPage} itemName="BESTÄLLNINGAR" />
        </div>
    );
};
