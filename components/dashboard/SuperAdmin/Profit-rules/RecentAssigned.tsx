            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                <div className="lg:col-span-2 bg-white rounded-lg shadow-[0px_0px_14px_0px_rgba(0,0,0,0.08)] p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-5 sm:mb-6">
                        <div>
                            <h2 className="text-lg sm:text-xl font-bold text-[#1A1C1C]">Senaste tilldelningar</h2>
                            <p className="text-[#78716C] text-xs sm:text-sm mt-0.5 sm:mt-1">Senaste grupper kopplade till vinstnivåer</p>
                        </div>
                        <button
                            type="button"
                    ...
                    {isLoading ? (
                        <div className="text-center py-8 text-stone-500 text-sm">Laddar sammanfattning...</div>
                    ) : recentCampaigns.length === 0 ? (
                        <div className="text-center py-8 text-stone-500 text-sm">Inga kampanjsammanfattningar hittades.</div>
                    ) : (
                        <div className="space-y-3 sm:space-y-4">
                            {recentCampaigns.map((item: any, index: number) => {
                                const groupName = item.groupId?.name || item.name || "Grupp";
                                ...
                                const tierName = percentageNum !== undefined && percentageNum !== null
                                    ? `${percentageNum}% Nivå`
                                    : tierObj?.name ? tierObj.name : "Otilldelad nivå";

                                const dateToUse = item.tierAssignDate || item.updatedAt || item.createdAt;
                                const assignedDate = dateToUse
                                    ? `Tilldelad ${new Date(dateToUse).toLocaleDateString("sv-SE", { month: "short", day: "numeric" })}`
                                    : "Nyligen tilldelad";
                ...
                <div className="bg-[#F59E0B1A] rounded-2xl sm:rounded-[32px] shadow-[0px_0px_14px_0px_rgba(0,0,0,0.08)] p-6 sm:p-8 flex items-center justify-center relative overflow-hidden min-h-36 sm:min-h-auto">
                    ...
                    <div className="relative z-10 text-center">
                        <div className="text-[#F59E0B] text-center text-xs sm:text-sm mb-1 sm:mb-2 font-medium">Total distribuerad vinst</div>
                        <div className="text-2xl sm:text-4xl text-center font-bold text-[#FBBF24]">
                            {isProfitLoading ? "Laddar..." : `SEK ${totalProfit.toLocaleString()}`}
                        </div>
                    </div>
                </div>
            </div>
