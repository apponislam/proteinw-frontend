...
const AdminList = () => {
    ...
    const handleApprove = async (adminId: string, adminName: string) => {
        ...
        const toastId = toast.loading(`Godkänner administratör ${adminName}...`);
        try {
            await approveAdmin(adminId).unwrap();
            toast.success(`Administratör ${adminName} godkändes framgångsrikt!`, { id: toastId });
        } catch (err: any) {
            toast.error(err?.data?.message || "Misslyckades med att godkänna administratör", { id: toastId });
        } finally {
            setApprovingId(null);
        }
    };
    ...
    if (isLoading) {
        return <div className="mt-8 text-center text-[#78716C] py-10">Laddar administratörer...</div>;
    }

    return (
        <div className="mt-6 sm:mt-8 bg-white p-4 sm:p-6 rounded-xl shadow-[0px_0px_14px_0px_rgba(0,0,0,0.08)]">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
                <h2 className="text-lg sm:text-xl font-bold text-[#1A1C1C]">Systemansvariga</h2>
            </div>
                ...
                <table className="w-full text-left min-w-160">
                    <thead>
                        <tr className="bg-[#FAFAF9]">
                            <th className="px-4 py-3 text-[#78716C] text-xs font-medium uppercase tracking-wider">ADMINISTRATÖRSNAMN</th>
                            <th className="px-4 py-3 text-[#78716C] text-xs font-medium uppercase tracking-wider">TILLDELADE GRUPPER</th>
                                        ...
                                        <td className="px-4 py-4 relative">
                                            {Array.isArray(admin.groupName) ? (
                                                admin.groupName.length > 0 ? (
                                                    <div className="relative group/paper inline-block cursor-pointer">
                                                        ...
                                                        <div className={`absolute left-0 ${isLastRow ? "bottom-full mb-2" : "top-full mt-2"} hidden group-hover/paper:flex flex-col gap-1.5 z-50 p-3 bg-white rounded-xl shadow-2xl border border-stone-200 min-w-44 max-w-xs animate-in fade-in zoom-in-95 duration-150`}>
                                                            <div className="text-[10px] font-bold text-[#78716C] uppercase tracking-wider mb-1">
                                                                Tilldelade grupper ({admin.groupName.length})
                                                            </div>
                                                    ...
                                                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-200 text-gray-700">OTILLDELAD</span>
                                                )
                                            ) : (
                                                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${admin.groupName ? "bg-[#D97706] text-white" : "bg-gray-200 text-gray-700"}`}>{admin.groupName || "OTILLDELAD"}</span>
                                            )}
                                        </td>
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    title="Visa administratörsdetaljer"
                                                >
                                                    <Eye size={16} />
                                                </button>
                                                <button
            ...
            <Pagination meta={meta} onPageChange={setPage} itemName="ADMINISTRATÖRER" />
